...
const logoTextColor = (isHomePage || isProgramsPage) && !isScrolled ? 'text-[#FBFBF9]' : 'text-[#1C1917]';  
const subTextColor = (isHomePage || isProgramsPage) && !isScrolled? 'text-[#D6C0A6]' : 'text-[#57534E]';
  const navTextColor = (isHomePage || isProgramsPage) && !isScrolled ? 'text-[#FBFBF9]/80 hover:text-[#FBFBF9]' : 'text-[#57534E] hover:text-[#1C1917]';
  const activeNavColor = (isHomePage || isProgramsPage) && !isScrolled ? 'text-[#FBFBF9]' : 'text-[#1C1917]';
  ...
  isScrolled
    ? 'glass py-4'
    : (isHomePage || isProgramsPage)
    ? 'bg-gradient-to-b from-black/50 to-transparent py-6'
    : 'bg-[#FBFBF9]/90 backdrop-blur-sm py-6'
}`}
      >
          ...
          <nav className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-5">
              ...
              <button
                ...
                className={`relative p-2 transition-colors duration-300 ${
                  (isHomePage || isProgramsPage) && !isScrolled ? 'text-[#FBFBF9] hover:text-[#D6C0A6]' : 'text-[#1C1917] hover:text-[#D6C0A6]'
                }`}
            ...
            <div className="lg:hidden flex items-center gap-2">
              ...
              <button
                onClick={() => setIsCartOpen(true)}
                className={`relative p-2 transition-colors duration-300 ${
                  (isHomePage || isProgramsPage) && !isScrolled ? 'text-[#D6C0A6]' : 'text-[#D6C0A6]'
                }`}
                data-testid="mobile-cart-btn"
              >
                <ShoppingCart className="w-5 h-5" />
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-[#D6C0A6] text-[#D6C0A6] w-5 h-5 rounded-full text-xs flex items-center justify-center font-medium">
                    {cartCount}
                  </span>
                )}
              </button>
              ...
              >
                {isMobileMenuOpen ? (
                  <X className={`w-6 h-6 ${(isHomePage || isProgramsPage) && !isScrolled ? 'text-[#D6C0A6]' : 'text-[#D6C0A6]'}`} />
                ) : (
                  <Menu className={`w-6 h-6 ${(isHomePage || isProgramsPage) && !isScrolled ? 'text-[#D6C0A6]' : 'text-[#D6C0A6]'}`} />
                )}
              </button>
            </div>
          </nav>
